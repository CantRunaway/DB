...
phone = CASE WHEN ? != hex(aes_encrypt('?', '키 값')) AND ? IS NOT NULL THEN hex(aes_encrypt('?', '키 값')) ELSE phone END,
account = CASE WHEN ? != hex(aes_encrypt('?', '키 값')) AND ? IS NOT NULL THEN hex(aes_encrypt('?', '키 값')) ELSE account END,
...
#사용자 정보 조회 - 리스트
Select user_id, `name`, grade, convert(aes_decrypt(unhex(phone), '키 값') using utf8) as 'phone', convert(aes_decrypt(unhex(account), '키 값') using utf8) as 'account', date_format(birth, '%Y-%m-%d') as birth, wt.work_type_name, b.bank_name from User u
...
select user_id, convert(aes_decrypt(unhex(account), '키 값') using utf8) as account, convert(aes_decrypt(unhex(phone), '키 값') using utf8) as phone from user;
